...
class DAO
{
	...
	public function __construct() {
		...
		{	// Phase 1 : connexion de l'application cliente au SGBD MySQL
			$this->cnx = mysqli_connect ($PARAM_HOTE , $PARAM_USER , $PARAM_PWD);
			// Phase 2 : sélection de la base de données mrbs :
			mysqli_select_db ($this->cnx, $PARAM_BDD);
			return true;
		}
		catch (Exception $ex)
	...
	{	global $cnx;
		...
		$jdd = mysqli_query ($this->cnx, $req);
		$nbReponses = mysqli_num_rows ($jdd);
	
		$reponse = "inconnu";
		if ($nbReponses > 0)
		{	$ligne = mysqli_fetch_array ($jdd);
			$level = $ligne['level'];
	...
	{	// préparation de la requete de recherche des réservations sans digicode
		...
		$jdd = mysqli_query ($this->cnx, $req1);
